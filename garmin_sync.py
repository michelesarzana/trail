...
print("Scaricando attività da Garmin...")
acts = garmin_get("https://connectapi.garmin.com/activitylist-service/activities/search/activities?limit=100&start=0")
print(f"  {len(acts)} attività trovate")
...
print(f"  {len(recent)} ultimo anno, {len(last90)} ultimi 90gg")
...
out = {
    ...
    'records': {
        ...
        'min_avg_hr': int(min((a.get('averageHR', 999) for a in recent if a.get('averageHR', 0) > 0), default=0)),
    },
    'by_sport': by_sport,
...
print(f"performance.json generato: {len(recent)} attività, {len(by_sport)} sport")
